...
var all_adjectives = {
  ...
  whimsical: ["whimsical", "silly", "drunken", "goofy", "funny", "weird", "strange", "odd", "playful", "clever", "boastful", "breakdancing",
              ...
              "brainwashed"],
  shiny: ["sapphire", "opal", "silver", "gold", "platinum", "ruby", "emerald", "topaz", "diamond", "amethyst", "turquoise", "starlit",
          "moonlit", "bronze", "metal", "jade", "amber", "garnet", "obsidian", "onyx", "pearl", "copper", "sunlit", "brass", "brassy",
          "metallic"],
...
function getAdj(x) {
  return all_adjectives[x] || all_adjectives.scientific;
}

// Pulls noun out of array using random number sent from generator
function getNoun(y) {
  return all_nouns[y] || all_nouns.scifi;
}
...
// number of columns and row height of our moving pizzas
var pizzaCols      = 8;
...
// the flag used to avoid multiple requestAnimationFrames
var pizzaTicking = false;

// onScroll handler - saves scroll position and requests animation to happen
function pizzaOnScroll() {
  pizzaScrollTop = document.body.scrollTop / 1250;
  pizzaRequestTick();
}

// tries to request animation frame unless there's one already in progress
...
// prepare our scroll handler which will request next animation frame
window.addEventListener('scroll', pizzaOnScroll);

// Generates the sliding pizzas when the page loads.
document.addEventListener('DOMContentLoaded', function() {
  var cols       = pizzaCols;
  var s          = pizzaRowHeight;
  var pizzasRoot = document.getElementById("movingPizzas1");

  // we assume that user's screen does not grow under us
  var maxHeight = screen.height;

  // hence we can determine how many pizza rows to generate
  var pizzaRows = Math.ceil(maxHeight / pizzaRowHeight);

  // and having predefined number of columns we how many pizzas to generate
  // it is much smaller number than 200 (unless you run this on 51200 vertical resolution screen)
  var pizzaTotal = pizzaRows * cols;

  // initialize our pizzas array (we'll use it directly instead of querying DOM)
  ...
  for (i = 0; i < pizzaTotal; i++) {
    ...
    // remember this one and add it to parent element
    pizzaItems[i] = elem;
